...
function getVehicleStatusStyle(status: VehicleStatus): {
  ...
  switch (status) {
    ...
    case "rented": // Added 'rented' from DB schema
...
export default function VehiclesClient({ initialVehicles }: VehiclesClientProps) {
  ...
  const filteredVehicles = useMemo(() => {
    if (!searchQuery.trim()) return initialVehicles;
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredVehicles.map((vehicle) => {
                  ...
                  const healthStyle = getStatusBadgeStyle(healthStatus);
                  const vehicleStatusStyle = getVehicleStatusStyle(vehicle.status as VehicleStatus);
                  return (
                      <tr
                        key={vehicle.id}
